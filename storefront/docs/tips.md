...
- 「線上騎乘體驗」已接上共用檔，Try it 在 scenario-2 頁內示範（key `ride`，從山路挑戰的選車開始，註冊在 scenario-2 的 app 原始碼）。目前「體驗報告與結帳」的項目仍用 `href` 直接連結（見下表）；如果你的頁面已經接上共用檔並註冊了動作，把該項目的 `href` 拿掉、改填 `page`，Try it 就會改成在頁內示範。
