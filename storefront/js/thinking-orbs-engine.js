/* thinking-orbs 0.3.1 engine — MIT © Jakub Antalik（見 thinking-orbs-LICENSE.txt）
   原檔是 ES module。這裡包成 IIFE 後掛到 window：
   一般 <script> 就能載入（file:// 直接開檔也不會被 CORS 擋），
   而且壓縮後的頂層變數（含一個 $）不會和頁面自己的變數撞名。*/
(function () {
function U(n, s, t) {
  return n + (s - n) * t;
}
